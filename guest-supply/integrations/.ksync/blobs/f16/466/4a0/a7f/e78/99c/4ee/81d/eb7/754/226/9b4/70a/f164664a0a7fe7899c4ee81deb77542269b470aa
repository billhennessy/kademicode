...
function processRow(BusinessUnit,InvoiceNo,AccountNo,ProductID, QtyOrder , QtySold , Price , Sales , SalesmanGP$ , ActualGP$ ,BackOrder,DropShip, Discount , Misc , Rebate , Restock ,OrderNo,OrderLine, Surcharge ) {
  ...
  */
  ...
  //var orderLine = formatter.toInteger(OrderLine);
  var orderLine = formatter.toString(OrderLine);
  var quantitySold = formatter.toInteger(isNaN(QtyOrder ? 0 : QtyOrder));
  var attributedTo = '';
  var periodFrom = '';
...
if( amount === 0){
  ...
  fields.put('orderline', formatter.toInteger(orderLine));
   db.fields = fields;
